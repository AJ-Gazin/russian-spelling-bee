...
def test_get_puzzle_by_id(client):
    cur = client.get("/puzzle/current").json()
    pid = cur["id"]
    r = client.get(f"/puzzle/{pid}")
    assert r.status_code == 200
    assert r.json()["id"] == pid


def test_get_missing_puzzle_404(client):
    r = client.get("/puzzle/9999999")
...
def test_guess_accepted_and_already_found(client):
    ...
    r = client.post(f"/puzzle/{pid}/guess", json={"form": form, "found_lemmas": []})
    assert r.status_code == 200
    j = r.json()
...
def test_guess_not_in_set(client):
    ...
    r = client.post(f"/puzzle/{pid}/guess", json={"form": "автомобилестроение"})
    assert r.status_code == 200
...
def test_guess_unparseable(client):
    cur = client.get("/puzzle/current").json()
    pid = cur["id"]
    r = client.post(f"/puzzle/{pid}/guess", json={"form": "—"})
    assert r.status_code == 200
